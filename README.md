...
字体使用角色已经拆分为 3 类（全局配置）：

- `ui.fonts.serif`：正文主字体（body / prose 主文）
- `ui.fonts.accent`：强调字体（引言、元信息、装饰文案）
- `ui.fonts.mono`：等宽字体（代码块、内联 code）

默认配置位于 `src/data/settings/ui.json` 的 `fonts` 字段。你可以：

1. 在 `src/styles/global.css` 继续新增或修改 `@font-face` 注册
2. 在 `src/data/settings/ui.json` 里把 `ui.fonts.*` 改为你注册后的 `font-family` 名称（可带 fallback）
...
{
  "fonts": {
    "serif": "\"Your Serif\", \"Noto Serif SC\", serif",
    "accent": "\"Your Accent\", \"LXGW WenKai Lite\", serif",
